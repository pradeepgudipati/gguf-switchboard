...
Search prints total system RAM, NVIDIA VRAM, and their combined capacity before an aligned table with SUPPORTED/FIT/SPEED/BALANCED/PRECISION columns:
...
| Column | Description |
|--------|-------------|
| `SUPPORTED` | Yes/No: standalone-eligible repo with at least one fitting named quant |
...
When a repo has SUPPORTED=No (no fitting named quant, or an auxiliary-only repo), SPEED/BALANCED/PRECISION show `-` and QUANT is empty — the repo is listed but no recommendations are made.
