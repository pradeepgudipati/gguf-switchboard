...
| Chat Completions tool calling | Supported | Passed through when the model and llama.cpp support it; load-time tool-call probe verifies capability |
...
| ModelFitPlanner | Supported | Opt-in `[fit]` section: bounded fallback ladder with profile caching |
| Memory-pressure eviction | Supported | Monitors system RAM; unloads at critical threshold |
| OOM context fallback | Supported | Auto-reduces `-c` on OOM-class failures down to `context_fallback_min` |
...
| Model management CLI | Supported | `models search`, `models files`, `models pull` for HF GGUF discovery, download, validation |
